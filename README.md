...
<p align="center">
  ...
  <a href="https://github.com/cossacklabs/themis/releases/latest"><img src="https://img.shields.io/badge/Server%20Platforms-Ubuntu%20%7C%20Debian%20%7C%20CentOS-green.svg" alt="Server platforms"></a>
...
Acra is still a product on a early development stage: any security tools require enourmous human efforts for validation of the methods, code, and finding possible infrastructural weaknesses. Although we do run Acra in production in several instances, we're continuously enhancing it as we go to everyone's benefit. And Acra still needs ruthless dissection of all of its properties to ensure that the provided security benefits are not rendered useless through implementation problems or increased complexity.
...
* Your app talks to **AcraProxy**, local daemon, via PostgreSQL/MySQL driver. **AcraProxy** emulates your normal PostgreSQL/MySQL database, forwards all the requests to **AcraServer** over a secure channel, and expects back plaintext output. 
* Then **AcraProxy** forwards it over the initial database connection to the application. It is connected to **AcraServer** via [Secure Session](https://github.com/cossacklabs/themis/wiki/Secure-Session-cryptosystem) or TLS, which ensures that all the plaintext goes over a protected channel. It is highly desirable to run **AcraProxy** via a separate user to compartmentalise it from the client-facing code. 
* **AcraServer** is the core entity that provides decryption services for all the encrypted envelopes that come from the database, and then re-packs database answers for the application. **AcraCensor** is part of AcraServer that allows to customize firewalling rules for all requests coming to the MySQL database.
...
- AcraServer passes each request through AcraCensor, which can be configured to blacklist or whitelist some queries. Allowed requests AcraServer forwards to the database. AcraCensor can be enabled only for MySQL databases.
...
- Read about using lightweight [HTTP web server](https://github.com/cossacklabs/acra/wiki/AcraConfigUI) we provide to manage AcraServer configuration in a simple fashion. 
- Read the notes on [security design](https://github.com/cossacklabs/acra/wiki/Security-design) and [intrusion detection](https://github.com/cossacklabs/acra/wiki/Intrusion-detection) to better understand what you get with using Acra and what is the threat model that Acra operates in. 
...
- Read about [logging format](https://github.com/cossacklabs/acra/wiki/Logging) that Acra support if you are using any SIEM system.  
