...
func init() {
	...
	additionalContext = []byte("ZXCMAkfCrCZzriauuJZ")
	encryptionKey, err = base64.StdEncoding.DecodeString("VUVDMgAAAC3iuBM2AtTwKgueCtJIEl3tPK0wzL8pmAiTfaOqXNF1xAX1oX9Q")
	if err != nil {
		log.Fatal(err)
	}
}
...
func Fuzz(data []byte) int {
	...
	acrastruct, err := acrastruct2.CreateAcrastruct(data, &keys.PublicKey{Value: encryptionKey}, additionalContext)
	if err != nil {
		panic(err)
	}

	var id int64
	err = dbConnection.QueryRow("insert into test (additionalContext, data) values ($1, $2) returning id;", additionalContext, acrastruct).Scan(&id)
	if err != nil {
		panic(err)
	}
	...
	var respZone []byte
	var respData []byte
	err = acraConnection.QueryRow("select additionalContext, data from test where id = $1", id).Scan(&respZone, &respData)
	if err != nil {
		panic(err)
	}
	if !bytes.Equal(respZone, additionalContext) {
		panic(fmt.Sprintf("respZone (%q) != additionalContext (%q)", respZone, additionalContext))
	}
