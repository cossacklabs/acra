			...
			if valuesList := insert.SelectStmt.GetSelectStmt().GetValuesLists(); len(valuesList) > 0 {
				for _, listItem := range valuesList {
					...
					for j, value := range items {
						...
						if changedValue, err := encryptor.encryptExpressionPgsql(ctx, value.GetAConst(), schema, columnName, bindPlaceholders); err != nil {
...
func (encryptor *QueryDataEncryptor) encryptExpressionPgsql(ctx context.Context, expr *pg_query.A_Const, schema config.TableSchema, columnName string, bindPlaceholder map[int]config.ColumnEncryptionSetting) (bool, error) {
	if schema.NeedToEncrypt(columnName) {
		...
		//	}
		//}
		//err := base.UpdateExpressionValue(ctx, expr, encryptor.dataCoder, setting, func(ctx context.Context, data []byte) ([]byte, error) {
		//	if len(data) == 0 {
		//		return data, nil
		//	}
...
func (encryptor *QueryDataEncryptor) encryptUpdateQuery(ctx context.Context, update *pg_query.UpdateStmt, bindPlaceholders map[int]config.ColumnEncryptionSetting) (bool, error) {
	...
	//}
	//
	//tables := base.GetTablesWithAliases(fromTables)
...
func (encryptor *QueryDataEncryptor) onReturning(ctx context.Context, returning sqlparser.Returning, fromTables sqlparser.TableExprs) error {
	...
	for _, item := range returning {
		...
		columnInfo, err := base.FindColumnInfo(fromTables, colName, encryptor.schemaStore)
		if err != nil {
			querySelectSettings = append(querySelectSettings, nil)
			continue
		}
...
func (encryptor *QueryDataEncryptor) encryptUpdateValues(ctx context.Context, update *sqlparser.Update, values []decryptor.BoundValue) ([]decryptor.BoundValue, bool, error) {
	...
	tables := base.GetTablesWithAliases(update.TableExprs)
